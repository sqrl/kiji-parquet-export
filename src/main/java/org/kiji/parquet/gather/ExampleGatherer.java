...
import org.kiji.parquet.ExampleRecord;
...
public class ExampleGatherer
  ...
  public void gather(KijiRowData row, GathererContext<AvroKey<ExampleRecord>, LongWritable> context)
    ...
    // when counting.
    NavigableMap<Long, ExampleRecord> counts = row.getValues(COLUMN_FAMILY, COLUMN_QUALIFIER);
    for (ExampleRecord key : counts.values()) {
      mRecord.setIdField(key.getIdField());
      mRecord.setTimestamps(key.getTimestamps());
